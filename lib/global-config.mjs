...
export async function writeCatalog(env = {}, products, fetchImpl = fetch) {
  ...
  let response;
  try {
    response = await fetchImpl(`${WRITE_HOST}/${config.id}/items${query}`, {
      method: 'PATCH',
      headers: {
      ...
      body: JSON.stringify({
        items: [{ operation: 'upsert', key: CATALOG_KEY, value: products }],
      }),
    });
  } catch {
    return { ok: false, reason: 'unreachable' };
  ...
  // Whatever Vercel says here describes OUR configuration, not a secret, and
  // it is the difference between a diagnosis and a guess. Carrying it out is
  // worth more than the tidiness of a generic message.
  let detail = null;
  try {
    const body = await response.json();
    detail = body?.error?.message ?? body?.error?.code ?? null;
  } catch {
    // Not every failure comes back as JSON; the status alone still helps.
  }
